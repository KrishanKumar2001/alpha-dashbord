...
const Attendance = () => {
  ...
  //     try {
  //       const response = await axios.get(`http://106.51.76.167:8080/timesheet/${userId}`);
  ...
  const handleSort = (sortBy) => {
    ...
    const getMonthOptions = (apiData) => {
      ...
      const attendanceMonths = apiData.attendances.map(entry => new Date(entry.date.slice(0, 7)));
